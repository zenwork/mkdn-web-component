...
fixture`mkdn-story - simple formatting`
	.page`http://localhost:8080/demo/mkdnstory/simple-formatting.html`;

test('component with content is present and formatted ', async t => {
	//find title
	let story = Selector(() => document.querySelector('mkdn-story').shadowRoot.querySelectorAll('section > div > h1'),
	...
	//find sub-titles
	let subtitles = Selector(() => document.querySelector('mkdn-story')
		.shadowRoot
		.querySelectorAll('section > div > h2'),
	                         {visibilityCheck:true});
